...
def merge_sort(lst):
    ...
    [0, 1, 2, 3, 6, 9]
    """
    if len(lst) < 2:    # if one item, list is already sorted
        return lst

    mid = int(len(lst) / 2)         # define where to break list in two
    lst1 = merge_sort(lst[:mid])    # sort first half of list
    lst2 = merge_sort(lst[mid:])    # sort second half

    return merge_lists(lst1, lst2)  # put it back together and return
